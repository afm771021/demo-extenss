...
class Credits(models.Model):
    ...
    def recalculate_amortization_table(self, credit_id, req_id, type_req):
        ...
        for cred in cred_s:
            ...
            for notice in notices:
                ...
                for concept in concepts:
                    ...
                    if cred.cs == True or cred.af == True:
                        if concept.concept == 'capital':
                    ...
                    if type_req == 'amount':
                        ...
                        if credit.calculation_base=='360/360':
                            ...
                            if credit.af == True or credit.ap == True:
                                ...
                                print("entra al else de 360")
                                if credit.include_taxes:
                                    dr=(credit.interest_rate / 360 )
                                else:
                                    print("entra else sin taxes")
                                    dr=(credit.interest_rate / 360 ) * (1+(credit.vat_factor/100))
                                    print("dr", dr)
                                    print('credit.interest_rate',credit.interest_rate)
                                    print('credit.vat_factor',credit.vat_factor)
                                    print('credit.frequency_days', credit.frequency_days)
                            if credit.frequency_days == 30:
                                print("entra a 30")
                                dm=30
                                rate=(dr/100*30)
                        ...
                        if credit.af == True or credit.ap == True:
                            ...
                            print("entra al else")
                            print('credit.term',credit.term)
                            print('rate',rate)
                            #ra=new_balance_table
                            pay=new_balance_table/((1-(1/pow((1+(rate)),credit.term-new_pay_num)))/(rate))
                        
                        for amort in credit.amortization_ids:
                            print('amort.no_pay',amort.no_pay)
                            #print('i',i+1)
                            if amort.no_pay > new_pay_num2:
                                print('amort.id',amort.id)
                                id_amort = amort.id
                                amortization_ids = [(2, id_amort, 0)]
                                credit.amortization_ids = amortization_ids
                            else:
                                print('amort.initial_balance',amort.initial_balance)
                                print('amort.capital',amort.capital)
                                ra=amort.initial_balance-amort.capital
                                print('ra del else',ra)
                                print('ra_new_addentro', ra-capital_req)
...
class ExtenssCreditExpiryNotices(models.Model):
    ...
    date_payment = fields.Date(string='Date payment', tracking=True, translate=True)
        ...
        for expiry in expirys:
            ...
            for cred in creds:
                ...
                if expiry.date_payment:
                    ...
                    #days_mora = (date_act - reg.due_not_date).days
                    days_mora = (expiry.start_date_mora - expiry.due_not_date).days
